...
uint64_t lv1_peek64(uint64_t address) {
	system_call_1(SYSCALL_LV1_PEEK, address);
	return_to_user_prog(uint64_t);
}

void lv1_poke64(uint64_t address, uint64_t value) {
	system_call_2(SYSCALL_LV1_POKE, address, value);
}

uint64_t lv2_peek64(uint64_t address) {
	system_call_1(SYSCALL_LV2_PEEK, address);
	return_to_user_prog(uint64_t);
}
...
void lv2_poke64(uint64_t address, uint64_t value) {
	system_call_2(poke_syscall, address, value);
}
...
int reboot(void) {
	system_call_4(SYSCALL_SM_SHUTDOWN, 0x8201, 0, 0, 0);
	return_to_user_prog(int);
}

int console_write(const char* message, unsigned int length) {
	system_call_2(SYSCALL_CONSOLE_WRITE, (uint64_t)(uintptr_t)message, (uint64_t)length);
	return_to_user_prog(int);
}
...
int ring_buzzer(uint64_t arg1, uint8_t arg2, uint32_t arg3) {
	system_call_3(SYSCALL_RING_BUZZER, (uint64_t)arg1, (uint64_t)arg2, (uint64_t)arg3);
	return_to_user_prog(int);
}
